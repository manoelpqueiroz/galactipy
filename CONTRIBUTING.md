...
|           :metro: Template Internals            |                       `template-config`<br>`docker`<br>`github`<br>`gitlab`<br>`ci-template`<br>`inv-template`<br>`invoke-template`                       | Changes related to the behaviour of existing auxiliary tools for project management providing functional operations to developers.                                              |
|         :ballot_box: Settings & Options         |                      `opt`<br>`opts`<br>`option`<br>`options`<br>`startup`<br>`tool`<br>`tools`<br>`template-dep`<br>`template-deps`                      | Changes related to the configuration of existing tools<br>including the startup options defined in `cookiecutter.json` and tool configuration in the template `pyproject.toml`. |
...
|      :factory_worker: Template Maintenance      |                            `refactor`<br>`refactors`<br>`refactoring`<br>`rework`<br>`reworks`<br>`reworking`<br>`maintenance`                            | Minor adjustments in configuration and code to keep files updated with the latest best practices and tool/service APIs.                                                         |
|            :gear: Project Internals             |                                                 `hook`<br>`hooks`<br>`project-config`<br>`dev`<br>`devx`                                                  | Any changes to tools and services used by Galactipy developers, as well as updates to pre and post-gen hooks logic for template generation.                                     |
| :airplane_departure: Release Cycle Optimisation |                                                            `ci`<br>`pre-commit`<br>`renovate`                                                             | Improvements to how the project is set up for compilation and release.                                                                                                          |
|          :repeat: Design & Validation           |                                        `test`<br>`tests`<br>`testing`<br>`cov`<br>`coverage`<br>`flag`<br>`flags`                                         | Code integrity validations, unit tests and feature flag enabling/disabling.                                                                                                     |
|         :arrow_up: Dependencies Updates         |                                                `dependencies`<br>`dep`<br>`deps`<br>`devdep`<br>`devdeps`                                                 |                                                                                                                                                                                 |
|              :pouring_liquid: Nits              |                                     `nit`<br>`nits`<br>`typo`<br>`typos`<br>`jinja-fix`<br>`rendering`<br>`markdown`                                      | Minor changes to configuration and text with the intent of improving file aesthetics.                                                                                           |
