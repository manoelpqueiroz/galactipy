...
| :---------------------------------------------: | :-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------: | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
|             :milky_way: Components              |                                          `feature`<br>`features`<br>`expand`<br>`expansion`<br>`expansions`<br>`enhance`<br>`enhancement`<br>`enhancements`<br>`zero-config`<br>`zconf`                                           | Introduction of new functionality to users or improvements to existing features directly affecting usage of template files once projects are generated.                      |
|        :metro: Components Configuration         |       `template-config`<br>`tool-config`<br>`tool-behavior`<br>`tool-behaviour`<br>`docker`<br>`github`<br>`gitlab`<br>`ci-template`<br>`inv-template`<br>`invoke-template`<br>`renovate-template`<br>`pre-commit-template`       | Changes related to the behaviour of existing auxiliary tools for project management providing functional operations to developers.                                           |
|         :ballot_box: Settings & Options         |                                                            `opt`<br>`opts`<br>`option`<br>`options`<br>`setting`<br>`settings`<br>`pyproject`<br>`zensical`<br>`toml`                                                             | Changes related to the configuration of existing tools including the startup options defined in `cookiecutter.json` and tool configuration in the template `pyproject.toml`. |
|           :newspaper2: Utility Files            |                       `util`<br>`utils`<br>`utility`<br>`utilities`<br>`content`<br>`contents`<br>`template-documentation`<br>`template-doc`<br>`template-docs`<br>`template-policy`<br>`template-policies`                       | Changes in non-operational files (i.e., text files) used to convey information within projects.                                                                              |
|      :hatching_chick: Template Generation       | `hook`<br>`hooks`<br>`cookiecutter`<br>`cookiecutter-var`<br>`cookiecutter-vars`<br>`cookiecutter-variable`<br>`cookiecutter-variables`<br>`startup`<br>`var`<br>`vars`<br>`jinja`<br>`jinja-conditional`<br>`jinja-conditionals` | Changes that have direct impact on how the Cookiecutter template is generated, either visible for endusers or not.                                                           |
|      :factory_worker: Template Maintenance      |    `refactor`<br>`refactors`<br>`refactoring`<br>`rework`<br>`reworks`<br>`reworking`<br>`reorg`<br>`reorganization`<br>`reorganisation`<br>`reorganize`<br>`reorganise`<br>`maintenance`<br>`template-dep`<br>`template-deps`    | Adjustments unrelated to tool/service behaviour aimed at keeping template files updated with the latest best practices in software development.                              |
|                 :toolbox: Fixes                 |                                                                    `bug`<br>`bugfix`<br>`fix`<br>`hotfix`<br>`patch`<br>`patches`<br>`patching`<br>`critical`                                                                     | Fixes for wrongful behaviour (i.e., bugs) that either renders template dysfunctional once generated or prevent template generation at all.                                   |
|            :gear: Project Internals             |                                                                 `project-config`<br>`dev`<br>`devx`<br>`dependencies`<br>`dep`<br>`deps`<br>`devdep`<br>`devdeps`                                                                 | Any changes to tools and services used by Galactipy developers, as well as updates to pre and post-gen hooks logic for template generation.                                  |
| :airplane_departure: Release Cycle Optimisation |                                                                                          `ci`<br>`invoke`<br>`pre-commit`<br>`renovate`                                                                                           | Improvements to how the project is set up for compilation and release.                                                                                                       |
|             :100: Template Cohesion             |                                                                            `test`<br>`tests`<br>`testing`<br>`cov`<br>`coverage`<br>`flag`<br>`flags`                                                                             | Code integrity validations, unit tests and changes related to code coverage and regressions.                                                                                 |
|            :pie: Features-in-waiting            |                                                                                                         `flag`<br>`flags`                                                                                                         | Inclusion of feature flag files not yet cleared for release.                                                                                                                 |
|              :pouring_liquid: Nits              |                                           `nit`<br>`nits`<br>`typo`<br>`typos`<br>`words`<br>`wording`<br>`layout`<br>`order`<br>`anchors`<br>`jinja-fix`<br>`rendering`<br>`markdown`                                            | Minor changes to configuration and text with the intent of improving file aesthetics.                                                                                        |
|       :frame_photo: Project Presentation        |                                                                                          `readme`<br>`documentation`<br>`doc`<br>`docs`                                                                                           | Changes to content related to promoting the project and highlighting its features.                                                                                           |
