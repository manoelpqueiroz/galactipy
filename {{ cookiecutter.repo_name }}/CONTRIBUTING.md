...
|              :toolbox: Fixes               |                                                             `bug`<br>`bugfix`<br>`fix`<br>`hotfix`<br>`security`<br>`sec`<br>`critical`<br>`leak`<br>`injection`                                                             | Fixes for wrongful behaviour (i.e., bugs).                                                                                                                             |
...
|         :scrolls: Project Policies         |                                  `policy`<br>`policies`<br>`rule`<br>`rules`<br>`milestone`<br>`milestones`<br>`epic`<br>`epics`<br>`roadmap`<br>`template`<br>`templates`<br>`templating`                                   | Changes that altered project rules and/or project-specific documentation.                                                                                              |
...
|           :toolbox: Fixes           |        `bug`<br>`bugfix`<br>`fix`<br>`fixed`<br>`hotfix`<br>`security`<br>`sec`<br>`critical`<br>`leak`<br>`injection`         |
|      :up: Dependencies Updates      |                                               `dependencies`<br>`dep`<br>`deps`                                                |
